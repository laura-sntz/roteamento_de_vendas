...
def carregar_dados():
    ...
    try:
        ...
        matriz_distancias_sensibilidade = pd.read_csv('matriz_distancias_sensibilidade.csv', index_col=0)
        pontos_de_visita_sensibilidade = pd.read_csv('pontos_de_visita_sensibilidade.csv')
        with open('resultados_branch_and_bound_sensibilidade.json', 'r') as f:
            resultados_bnb_sensibilidade = json.load(f)
        with open('geometrias_rotas_sensibilidade.json', 'r') as f:
...
def dashboard_visualizacao_rota(pontos_de_visita, resultados_bnb, geometrias_rotas):
    ...
    # CORREÇÃO: Pegar dados da cidade inicial pelo índice
    ...
    # CORREÇÃO: Iterar pela lista de índices para ter a ordem correta
    for idx_parada in range(len(rota_indices)):

        # O índice real da cidade no CSV
        origem_idx = rota_indices[idx_parada]

        # O índice real da próxima cidade
        ...
        # CORREÇÃO: Pegar os dados da cidade atual pelo índice real
        row = pontos_de_visita.iloc[origem_idx]

        # CORREÇÃO: Usar o 'idx_parada' (0, 1, 2...) para o popup
        if idx_parada == 0:
            cor = 'green'
            popup = f"Início/Fim: {row['cidade']}"
        else:
            cor = 'blue'
            popup = f"Parada {idx_parada}: {row['cidade']}"  # <--- CORRIGIDO
...
def mapa_sensibilidade(pontos_df, resultados_dict, geometrias_dict, map_title, map_color):
    ...
    folium_static(m, width=380, height=350)
...
def dashboard_comparativo_e_validacao(matriz_distancias, pontos_de_visita, resultados_bnb,
    ...
    # ... (Esta função está correta, mas foi atualizada na sua última interação para incluir os nomes da rota heurística) ...
